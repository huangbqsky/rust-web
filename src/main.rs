...
async fn main() -> Result<()> {
    ...
    cmd_sender.send(Command::Start).await.unwrap_or_default();
            ...
            Command::Stop => {
                match server {
                    Ok(accept_loop_handle) => {
                        ...
                        server = Err(Error::from(ErrorKind::NotConnected));
                        println!("stopped");
                    }
                    Err(ref err) => {
                        println!("stopped already");
                        println!("stopped server failed: {}", err);
                    }
                }
            }
            Command::Quit => {
